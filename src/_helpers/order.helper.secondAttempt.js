...
const findSweetPackCombination = (numberOfSweets, sweetPackSizes) => {
    let newTotal = numberOfSweets;
    let sweetPackTotals = {};

    for (let i = sweetPackSizes.length - 1; i >= 0; i--) {
        while (newTotal - sweetPackSizes[i] >= 0) {
            newTotal -= sweetPackSizes[i];
            if (sweetPackTotals[sweetPackSizes[i]]) {
                sweetPackTotals[sweetPackSizes[i]] += 1;
            } else {
                sweetPackTotals[sweetPackSizes[i]] = 1;
            }
        }
